...
class ff_xmlPackage
{
	...
	function import($filename)
	{
		...
		if (!$filename) {
			$this->setError(_FACILEFORMS_XML_MISSFNAME);
			return false;
		} // if
		$this->filename = $filename;
		if (!($fp = fopen($this->filename, "r")))   {
			$this->setError(_FACILEFORMS_XML_ERROPENF.' '.$this->filename);
			return false;
		} // if
	...
	function setError($message, $inparse = false)
	{
		...
		if ($level>0) $this->error .= _FACILEFORMS_XML_ELEMENT." '".$this->element[$level-1]."' ";
		if ($inparse) $this->error .= _FACILEFORMS_XML_ATLINE.' '.xml_get_current_line_number($this->parser).' ';
...
function &_ff_xmlGetPackage($parser)
{
	...
	die(_FACILEFORMS_XML_REFMISSED." (".__FUNCTION__.")");
} // _ff_xmlGetPackage
...
function _ff_xmlStartElement($parser, $key, $attr)
{
	...
	if (!array_key_exists('elem', $curr) ||
		...
		$pkg->setError(_FACILEFORMS_XML_UNEXPELEM." '$key'", true);
		return;
	} // if
	...
	if (is_array($curr)) { // is not leaf
		...
		foreach ($attr as $key => $value) {
			if (!array_key_exists($key, $a)) {
				$pkg->setError(_FACILEFORMS_XML_UNEXPATTR." '$key'", true);
				return;
			} // if
			$value = trim($value);
			eval($a[$key]);
...
function _ff_xmlCharData($parser, $value)
{
	...
	if (!is_array($curr))
		...
		if (array_key_exists('data', $curr))
			...
			$value = trim($value);
			if ($value != '') $pkg->setError(_FACILEFORMS_XML_UNEXPDATA.": ".$value, true);
		} // if
	} // if
} // _ff_xmlCharData
...
function _ff_xmlEndElement($parser, $key)
{
	...
	// check if expected
	if ($tag != $key) {
		$pkg->setError(_FACILEFORMS_XML_UNEXPCLOS." '$key'", true);
		return;
	} // if
