...
$ff_resnames = array(
	'ff_align', 'ff_border', 'ff_form',   'ff_frame', 'ff_mode', 'ff_name',
	'ff_page',  'ff_rmde',   'ff_target', 'ff_task',  'ff_top',  'ff_suffix'
);
...
function initFacileForms($mode = 'frontend')
{
	global $ff_mossite, $ff_comsite, $ff_config, $mosConfig_live_site;

	if ($ff_config->livesite) {
		$ff_mossite = str_replace('\\','/',$mosConfig_live_site);
		$len = strlen($ff_mossite);
		if ($len>0 && $ff_mossite{$len-1}=='/') $ff_mossite = substr($ff_mossite,0,$len-1);
	} else {
		$s = str_replace('\\','/',$_SERVER['PHP_SELF']);
		switch ($mode) {
			case 'frame':
				$x = '/components/com_facileforms/facileforms.frame.php';
				break;
			case 'backend':
				$x = '/administrator/index';
				break;
			default:
				$x = '/index';
		} // switch
		$p = strpos($s, $x);
		if ($p === false) die(' String "'.$x.'" not found in PHP_SELF: "'.$s.'"');
		$ff_mossite = 'http://'.$_SERVER['HTTP_HOST'].substr($s, 0, $p);
	} // if
	$ff_comsite = $ff_mossite.'/components/com_facileforms';
} // initFacileForms
...
class facileFormsConf {
	...
	var $images         = '{mossite}/components/com_facileforms/images';    // {ff_images} path
	var $uploads        = '{mospath}/components/com_facileforms/uploads';   // {ff_uploads} path
	...
	function bindRequest($request)
	{
		$arr = get_object_vars($this);
		while (list($prop, $val) = each($arr))
			$this->$prop = mosGetParam($request, $prop, $val);
	} // bindRequest
} // class facileFormsConf

class facileFormsMenus extends mosDBTable {
	...
	{
		$this->mosDBTable('#__facileforms_compmenus', 'id', $db);
	} // constructor
...
class facileFormsForms extends mosDBTable {
	var $id             = null;     // identifier
	var $package        = null;     // package name
	...
	{
		$this->mosDBTable('#__facileforms_forms', 'id', $db);
	} // constructor
...
class facileFormsElements extends mosDBTable {
...
*/
	...
	{
		$this->mosDBTable('#__facileforms_elements', 'id', $db);
	} // constructor
...
class facileFormsScripts extends mosDBTable {
	var $id             = null;     // identifier
	...
	{
		$this->mosDBTable('#__facileforms_scripts', 'id', $db);
	} // constructor
...
class facileFormsPieces extends mosDBTable {
	var $id             = null;     // identifier
	var $published      = null;     // is published
	var $package        = null;     // package name
	var $name           = null;     // function name
	...
	{
		$this->mosDBTable('#__facileforms_pieces', 'id', $db);
	} // constructor
...
class facileFormsRecords extends mosDBTable {
	var $id             = null;     // identifier
	...
	{
		$this->mosDBTable('#__facileforms_records', 'id', $db);
	} // constructor
...
class facileFormsSubrecords extends mosDBTable {
	var $id             = null;     // identifier
	...
	{
		$this->mosDBTable('#__facileforms_subrecords', 'id', $db);
	} // constructor
