<?php
/**
* Facile Forms - A Mambo Forms Application
* @version 1.4.4
* @package FacileForms
...
class HTML_facileFormsProcessor
{
	...
	var $option         = null;     // mos option
	var $itemid         = null;     // mos itemid
	...
	function HTML_facileFormsProcessor(
		...
		$form,          // form id
		$page = 1,      // page #
		$option = null, // mambo option
		$itemid = null, // mambo itemid
	...
	function expJsValue($mixed, $indent='')
	{
		...
		// not supported types
		if (is_object($arg)) return $indent."'"._FACILEFORMS_PROCESS_OBJECT."'";
		if (is_resource($arg)) return $indent."'"._FACILEFORMS_PROCESS_RESOURCE."'";
	...
	function loadBuiltins(&$library)
	{
		...
		if ($this->showgrid) {
			...
			$library[] = array('ff_showgrid',
				...
				"       document.body.appendChild(e);".nl().
				"   } // for".nl().
		...
		if ($this->inframe)
			$code .=
			"        parent.document.getElementById('ff_frame".$this->form."').style.height = totheight+'px';".nl().
	...
	function header()
	{
		...
		return
			...
			$this->expJsVar('ff_processor.option     ', $this->option).
			$this->expJsVar('ff_processor.itemid     ', $this->itemid).
	...
	function view()
	{
		...
		global $ff_config, $ff_version, $ff_comsite;
		...
		if (!$this->inframe) {
			...
			echo "<!-- Facile Forms V".$ff_version." (C) 2004-2005 by IBK Software AG.  All rights reserved. -->".nl();
			// END OF COPYRIGHT
		} // if
		...
		if ($icons > 0) {
			...
			$this->linkcode('ff_dispIconBorder', $library, $linked,
				...
				'{'.nl().
				'    element.style.border = "2px outset";'.nl().
				'} // ff_dispIconBorder'
			);
			if ($this->bury()) return;
		} // if
		...
		if (!$this->inline) {
			if ($this->inframe)
				$url = $ff_comsite.'/facileforms.frame.php';
			else
				switch ($this->runmode) {
					case _FF_RUNMODE_FRONTEND:
		...
		for($i = 0; $i < $this->rowcount; $i++) {
			...
			switch ($row->type) {
				...
				case 'Tooltip':
					...
					echo indentc(2).'<img src="'.$url.'" border="0"'.$class2.'/>'.nlc();
					echo indentc(1).'</div>'.nl();
					break;
				case 'Hidden Input':
				...
				case 'Graphic Button':
					...
					switch ($row->flag1) {
						...
						case 1: // below
							...
							echo indentc(5).'<img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" '.$attribs.'/><br/>'.nlc();
						...
						case 2: // above
							...
							echo indentc(5).'<img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" '.$attribs.'/>'.nlc();
							echo indentc(4).'</td></tr>'.nlc();
							echo indentc(3).'</table>.nlc()';
							break;
						case 3: // left
							...
							echo indentc(5).'<td><img id="ff_img'.$row->id.'" src="'.$data1.'"  border="0" '.$attribs.'/></td>'.nlc();
							echo indentc(4).'</tr>'.nlc();
							echo indentc(3).'</table>'.nlc();
							break;
				...
				case 'Icon':
					...
					switch ($row->flag1) {
						case 0: // none
							...
							echo indentc(3).'<img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" align="middle" '.$attribs.$class2.'/>'.nlc();
							echo indentc(2).'</span>'.nlc();
							break;
						case 1: // below
							echo indentc(2).'<table id="ff_elem'.$row->id.'" cellpadding="1" cellspacing="0" border="0" '.$swap.'>'.nlc();
							echo indentc(3).'<tr><td style="text-align:center;"><img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" align="middle" '.$attribs.$class2.'/></td></tr>'.nlc();
						...
						case 3: // left
							...
							echo indentc(3).$data2.' &nbsp;<img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" align="middle" '.$attribs.$class2.'/>'.nlc();
							echo indentc(2).'</span>'.nlc();
							break;
						default: // assume right
							echo indentc(2).'<span id="ff_elem'.$row->id.'" '.$swap.' style="vertical-align:middle;">'.nlc();
							echo indentc(3).'<img id="ff_img'.$row->id.'" src="'.$data1.'" border="0" align="middle" '.$attribs.$class2.'/>&nbsp; '.$data2.nlc();
							echo indentc(2).'</span>'.nlc();
							break;
					} // switch
					echo indentc(1).'</div>'.nl();
		...
		switch ($this->runmode) {
			case _FF_RUNMODE_FRONTEND:
				echo indentc(1).'<input type="hidden" name="option" value="'.$this->option.'"/>'.nl();
				if ($this->itemid != null && $this->itemid != '')
					echo indentc(1).'<input type="hidden" name="Itemid" value="'.$this->itemid.'"/>'.nl();
				if ($this->border != 0)
					echo indentc(1).'<input type="hidden" name="ff_border" value="'.$this->border.'"/>'.nl();
				if ($this->align != 1)
			...
			case _FF_RUNMODE_BACKEND:
				...
				echo indentc(1).'<input type="hidden" name="ff_form" value="'.$this->form.'"/>'.nl().
					 indentc(1).'<input type="hidden" name="ff_page" value="'.$this->page.'"/>'.nl().
					 indentc(1).'<input type="hidden" name="ff_task" value="submit"/>'.nl().
					 indentc(1).'<input type="hidden" name="ff_mode" value="'.$this->runmode.'"/>'.nl().
					 '</form>'.nl();
				break;
	...
	function sendEmailNotification()
	{
		...
		$fromname = $mosConfig_fromname.' - Facile Forms';
	...
	function submit()
	{
		global $database, $ff_config, $ff_comsite, $ff_mossite;
