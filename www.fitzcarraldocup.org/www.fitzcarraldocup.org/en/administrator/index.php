...
if (isset( $_POST['submit'] )) {
	...
	if (!$pass) {
		echo "<script>alert('Inserire una password'); document.location.href='index.php';</script>\n";
	} else {
		$pass = md5( $pass );
	}
	...
	if (@$my->id) {
		...
		if ( strcmp( $my->password, $pass ) || !$acl->acl_check( 'administration', 'login', 'users', $my->usertype ) ) {
			mosErrorAlert("Username, Password, o Livello di Accesso non corretti.  Riprova", "document.location.href='index.php'");
		}
		...
		mosErrorAlert("Username e Password non corrette, riprova", "document.location.href='index.php'");
	}
} else {
	initGzip();
