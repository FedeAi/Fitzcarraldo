...
if (!defined( '_JOS_QUICKICON_MODULE' )) {
	...
	<div id="cpanel">
		...
		quickiconButton( $link, 'module.png', 'Aggiungi nuovo contenuto' );
	
		$link = 'index2.php?option=com_content&sectionid=0';
		quickiconButton( $link, 'addedit.png', 'Gestione contenuti' );
	
		$link = 'index2.php?option=com_typedcontent';
		quickiconButton( $link, 'addedit.png', 'Gestione contenuti statici' );
	
		$link = 'index2.php?option=com_frontpage';
		quickiconButton( $link, 'frontpage.png', 'Gestione prima pagina' );
	
		$link = 'index2.php?option=com_sections&amp;scope=content';
		quickiconButton( $link, 'sections.png', 'Gestione sezioni' );
	
		$link = 'index2.php?option=com_categories&amp;section=content';
		quickiconButton( $link, 'categories.png', 'Gestione categorie' );
	
		$link = 'index2.php?option=com_media';
		quickiconButton( $link, 'mediamanager.png', 'Gestione media' );
	
		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_trash';
			quickiconButton( $link, 'trash.png', 'Gestione cestino' );
		}
	
		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_menumanager';
			quickiconButton( $link, 'menu.png', 'Gestione menu' );
		}
	
		if ( $my->gid > 24 ) {
			$link = 'index2.php?option=com_languages';
			quickiconButton( $link, 'langmanager.png', 'Gestione lingue' );
		}
	
		if ( $my->gid > 23 ) {
			$link = 'index2.php?option=com_users';
			quickiconButton( $link, 'user.png', 'Gestione utenti' );
		}
	
		if ( $my->gid > 24 ) {
			$link = 'index2.php?option=com_config&hidemainmenu=1';
			quickiconButton( $link, 'config.png', 'Configurazione Globale' );
		}
		?>
	</div>
	<?php
