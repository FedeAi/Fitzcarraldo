...
foreach ( $rows as $row ) {
	if ( $acl->acl_check( 'administration', 'manage', 'users', $my->usertype, 'components', 'com_users' ) ) {
		...
		$name 	= '<a href="'. $link .'" title="Modifica utente">'. $row->username .'</a>';
	} else {
		$name 	= $row->username;
	}
	?>
	<tr>
		...
		if ( $acl->acl_check( 'administration', 'manage', 'users', $my->usertype, 'components', 'com_users' ) ) {
			...
			<img src="images/publish_x.png" width="12" height="12" border="0" alt="Disconnetti" Title="Forza uscita utente" />
			</a>
			</td>
			<?php
		}
