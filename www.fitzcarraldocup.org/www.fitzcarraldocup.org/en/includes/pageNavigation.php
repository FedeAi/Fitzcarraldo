	...
	*/
	function writePagesLinks( $link ) {
		...
		if ($this_page > 1) {
			...
			$txt .= '<a href="'. sefRelToAbs( "$link&amp;limitstart=0" ) .'" class="pagenav" title="prima pagina">'. _PN_LT . _PN_LT . $pnSpace . _PN_START .'</a> ';
			$txt .= '<a href="'. sefRelToAbs( "$link&amp;limitstart=$page" ) .'" class="pagenav" title="pagina precedente">'. _PN_LT . $pnSpace . _PN_PREVIOUS .'</a> ';
		} else {
		...
		if ($this_page < $total_pages) {
			...
			$txt .= '<a href="'. sefRelToAbs( $link .'&amp;limitstart='. $page ) .' " class="pagenav" title="prossima pagina">'. _PN_NEXT . $pnSpace . _PN_RT .'</a> ';
			$txt .= '<a href="'. sefRelToAbs( $link .'&amp;limitstart='. $end_page ) .' " class="pagenav" title="ultima pagina">'. _PN_END . $pnSpace . _PN_RT . _PN_RT .'</a>';
		} else {
